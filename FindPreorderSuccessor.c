...
bt* findPreorderSuccessor(bt *root, bt **prev){
    if(root == NULL)
        return NULL;
    root->preorderSuccessor = *prev;
    (*prev) = root;
    findPreorderSuccessor(root->left,prev);
    findPreorderSuccessor(root->right,prev);
    return root;
}
...
void displayPreorderSuccessor(bt *root){
    if(root == NULL)
        return;
    if(root->preorderSuccessor != NULL)
        ...
        printf("NULL ");
    displayPreorderSuccessor(root->left);
    displayPreorderSuccessor(root->right);
}
int main(){
